...
module Data.Tree.DisjointIntervalTree
    (DisjointIntervalTree,
     empty, lookup, keys, insert, delete,
     runAllTests)
    where
...
data Ord k => DisjointIntervalTree k v = Tree !k Int (Maybe v) !(DisjointIntervalTree k v) !(DisjointIntervalTree k v) |
                                              Tip
...
key :: Ord k => DisjointIntervalTree k v -> k
key Tip = error "Tried to get key of tip"
key (Tree k _ _ _ _) = k

value :: Ord k => DisjointIntervalTree k v -> Maybe v
value Tip = Nothing
value (Tree _ _ v _ _) = v

depth :: Ord k => DisjointIntervalTree k v -> Int
depth Tip = 0
depth (Tree _ d _ _ _) = d

left, right :: Ord k => DisjointIntervalTree k v -> DisjointIntervalTree k v
left (Tree _ _ _ l _) = l
right (Tree _ _ _ _ r) = r

keys :: Ord k => DisjointIntervalTree k v -> [(k, k)]
keys Tip = []
...
lookup :: Ord k => k -> DisjointIntervalTree k v -> Maybe v
lookup key tree = case lookupNode key tree of
                    Nothing -> Nothing
                    Just x -> value x

delete :: Ord k => (k, k) -> DisjointIntervalTree k v -> DisjointIntervalTree k v
delete bounds tree = insertInBounds bounds Nothing tree

insert :: Ord k => (k, k) -> v -> DisjointIntervalTree k v -> DisjointIntervalTree k v
insert bounds v tree = insertInBounds bounds (Just v) tree

bounds :: Ord k => DisjointIntervalTree k v -> (k, k)
bounds tree = (key $ minNode tree, key $ maxNode tree)

-- Utility functions
lookupNode :: Ord k => k -> DisjointIntervalTree k v -> Maybe (DisjointIntervalTree k v)
...
insertInBounds :: Ord k => (k, k) -> (Maybe v) -> DisjointIntervalTree k v -> DisjointIntervalTree k v
insertInBounds (lowerBound, upperBound) v tree
    ...
    | otherwise =
        ...
        in
          insertValue lowerBound v $ insertValue upperBound valueForUpperBound clearedTree

insertValue :: Ord k => k -> (Maybe v) -> DisjointIntervalTree k v -> DisjointIntervalTree k v
...
cleanInterval :: Ord k => (k, k) -> DisjointIntervalTree k v -> DisjointIntervalTree k v
...
deleteStartingAt :: Ord k => k -> DisjointIntervalTree k v -> DisjointIntervalTree k v
...
balance :: Ord k => DisjointIntervalTree k v -> DisjointIntervalTree k v
balance Tip = Tip
...
leftRotate, rightRotate :: Ord k => DisjointIntervalTree k v -> DisjointIntervalTree k v
...
minNode, maxNode :: Ord k => DisjointIntervalTree k v -> DisjointIntervalTree k v
...
instance (Arbitrary k, Arbitrary v, Num k, Ord k, Eq v) => Arbitrary (DisjointIntervalTree k v) where
    arbitrary = sized tree'
        where
          tree' :: (Arbitrary k, Arbitrary v, Ord k, Num k) => Int -> Gen (DisjointIntervalTree k v)
          tree' 0 = return Tip
          tree' n | n > 0 = do
